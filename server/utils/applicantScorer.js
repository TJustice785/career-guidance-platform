// Automatic Applicant Scoring System
 ...
 */
function calculateApplicantScore(applicant, job) {
  let totalPoints = 0;
  let maxPoints = 0;
  const breakdown = {};
  ...
  breakdown.workExperience = {
    ...
    max: 25,
    details: `${yearsOfExperience} year(s) of experience, ${experience.length} position(s)`
  };
  ...
  breakdown.relevance = {
    ...
    max: 25,
    details: `Field: ${applicantField || 'N/A'}, Level: ${applicantEducation || 'N/A'}`
  };
...
function getRecommendation(score) {
  ...
  if (score >= 75) return 'Very Good Match - Recommended';
  if (score >= 60) return 'Good Match - Consider for Interview';
  if (score >= 40) return 'Potential Match - Review Carefully';
 ...
 */
function filterQualifiedApplicants(applicants, job) {
  return applicants
    .map(applicant => {
